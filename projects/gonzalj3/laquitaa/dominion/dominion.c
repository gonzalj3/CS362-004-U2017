...
void playAdventurer(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int temphand[MAX_HAND], int z ){
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)//JMG Found this Bug //introduced bug to not count gold as a resource
	  {
		  drawntreasure++;
		printf("we have found a treasure card\n");
	  }
	else{
	...
	printf("discardcount is now %d \n", state->discardCount[currentPlayer]);
      }
}

void playSmithy(struct gameState *state, int currentPlayer, int handPos){
	int i;
	for (i = 0; i < 3; i++) // JMG, found and fixed this bug//changed i < 3 to i <=3 which would have player draw 4 cards instead of 3
	{
	  drawCard(currentPlayer, state);
	}
